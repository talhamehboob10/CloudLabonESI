...
@EXPORT =
	...
	freenasExtentCreate freenasAuthInitCreate
	freenasTargetCreate freenasTargetExtentCreate
	freenasExtentDelete freenasAuthInitDelete
	freenasTargetDelete freenasTargetExtentDelete
...
#
# Exported resources (v2.0 API)
#
our $FREENAS_API_RESOURCE_IST_EXTENT  = "iscsi/extent";
our $FREENAS_API_RESOURCE_IST_AUTHI   = "iscsi/initiator";
our $FREENAS_API_RESOURCE_IST_TARGET  = "iscsi/target";
our $FREENAS_API_RESOURCE_IST_TGTGROUP= "iscsi/targetgroup";
our $FREENAS_API_RESOURCE_IST_ASSOC   = "iscsi/targetextent";
our $FREENAS_API_RESOURCE_POOL        = "pool";
our $FREENAS_API_RESOURCE_VOLUME      = "pool/dataset";
our $FREENAS_API_RESOURCE_SNAPSHOT    = "zfs/snapshot";
...
#
# Make a request via the FreeNAS v2.0 API.
...
sub freenasRequest($;$$$$$)
{
    ...
    if ($datap && !ref($datap) && ($datap eq "true" || $datap eq "false")) {
        $datastr = $datap;
    } else {
        my %data = $datap ? %$datap : ();
    ...
    my $url = "http://$server/api/v2.0/$resource/$paramstr";
    my %headers = (
	...
	if (!exists($res->{'content'}) ||
	    (!ref($res->{'content'}) && $res->{'content'} eq "null")) {
	    return {};
	}
	# XXX v2.0, content is JSON regardless of content-type
	...
	if (exists($res->{'content'}) &&
		...
		if (exists($cref->{'__all__'})) {
		    ...
		    $content = $cref->{'error'};
		} elsif (exists($cref->{'message'})) {
		    $content = $cref->{'message'};
		}
	    } elsif ($cref) {
		$content = $cref;
	    } else {
...
sub freenasVolumeList($;$)
{
    ...
    # 
    if ($inameinfo) {
	my $extinfo = freenasExtentList(1);
	foreach my $extkey (keys %$extinfo) {
	    my $ext = $extinfo->{$extkey};
	    if ($ext->{'path'} =~ /^zvol\/([-\w]+\/[-\w+]+)$/) {
		$inames{$1} = $extkey;
	    }
	}
    }
    ...
    # The pool/datasets API returns all attributes, but for inherited
    # values an entry doesn't exist and you would have to trace back up
    # the dataset hierarchy to find a value. I am too lazy for that.
    #
    ...
    #
    # The FreeNAS APIv2 allows you to request just volumes (zvols) by passing
    # the undocumented type=VOLUME argument. Note that the returned items
    # contain all the attributes so we could identify clones via the origin
    # attribute, but we just stick with the zfs command method we have always
    # used.
    #
    my $vinfo = freenasRequest($FREENAS_API_RESOURCE_VOLUME,
			       "GET", {"limit" => 0, "type" => "VOLUME"});
...
sub freenasVolumeCreate($$$;$$)
{
    ...
    # V2 expects size in bytes
    $size *= 1024 * 1024;

    #
    # XXX don't explicitly turn compression on/off, use the default from
    # the pool. Probably should be an explicit option.
    #
    my $msg;
    my $res = freenasRequest("$FREENAS_API_RESOURCE_VOLUME",
			     "POST", undef,
			     {"name" => "$pool/$volname",
			      "type" => "VOLUME",
			      "volsize" => "${size}",
...
sub freenasVolumeDesnapshot($$;$$$)
{
	...
	foreach my $sname (@snaps) {
		...
		my $msg;
		my $resource =
		    "$FREENAS_API_RESOURCE_SNAPSHOT/id/${pool}\%2F${sname}";
...
sub freenasVolumeClone($$$;$$)
{
    ...
    my $res = freenasRequest($resource, "POST", undef,
			     {"snapshot" => "${pool}/${snapshot}",
			      "dataset_dst" => "$pool/$nvolname"});
    if (!$res || $res eq JSON::PP::false) {
...
sub freenasVolumeCopy($$$;$)
{
    ...
    my $msg;
    my $resource = "$FREENAS_API_RESOURCE_SNAPSHOT/id/${pool}\%2F${snapshot}";
    ...
    $resource = "$FREENAS_API_RESOURCE_SNAPSHOT/id/${pool}\%2F${snapshot}";
    $res = freenasRequest($resource, "DELETE", undef, undef, undef, \$msg);
    if (!$res) {
	warn("*** WARNING: freenasVolumeCopy: ".
	     "delete of $snapshot failed:\n$msg");
...
sub volumeDestroy($$$$$) {
  ...
  retry:
    ...
    # we slept.
    #
    my $resource = "$FREENAS_API_RESOURCE_VOLUME/id/${pool}\%2F${volname}";
    my $msg;
	...
	if ($declone) {
	    my $msg;
	    my $resource =
		"$FREENAS_API_RESOURCE_SNAPSHOT/id/${pool}\%2F${snapshot}";
...
sub freenasAuthInitList() {
	...
	#
	# XXX not sure how this maps to the V1 initiators field.
	# For V1 it was always "ALL" for us, for V2 it is always
	# an empty array. So we map empty array to "ALL". If there
	# is anything in the array, freak out.
	#
	if (@{$ai->{'initiators'}} > 0) {
	    warn("*** WARNING: freenasAuthInitList: ".
		 "overwriting unexpected non-empty initiators array, ".
		 "bad things will likely happen!\n");
	}
	$ai->{'initiators'} = "ALL";

	#
	# XXX auth_network is an array in V2, a string in V1.
	# For us, it is always a single network, but maybe it could
	# be multiple? We map the first element to a string and
	# again freak out if there was more than one element.
	#
	if (@{$ai->{'auth_network'}} != 1) {
	    warn("*** WARNING: freenasAuthInitList: ".
		 "overwriting multi-element network array, ".
		 "bad things will likely happen!\n");
	}
	$ai->{'auth_network'} = $ai->{'auth_network'}[0];

	#
	# XXX we only have one authentication group so there is only one
	# "tag". But the code seems to want to use the tag to uniquely
	# identify an initiator. So we create a tag equal to the id.
	#
	$ai->{'tag'} = $ai->{'id'};
...
# N.B. this info is now returned by the iscsi/target resource.
#
...
sub freenasExtentCreate($$$$;$)
{
    ...
    $isro = ($isro eq "true") ? JSON::PP::true : JSON::PP::false;

    # XXX V2 only allows up to 15 char SNs, we generate 16 chars
    my $sn = substr(libvnode_blockstore::genSerial(), 0, 15);
...
#
sub freenasExtentDelete($;$)
{
    my ($eindex, $msgp) = @_;

    my $msg = "delete target API call failed";
    my $res = freenasRequest("$FREENAS_API_RESOURCE_IST_EXTENT/id/$eindex",
...
{
    my ($network, $cmask, $comment, $msgp, $tagp) = @_;

    my $msg = "create authorized initiator API call failed";
    my $res = freenasRequest($FREENAS_API_RESOURCE_IST_AUTHI,
			     "POST", undef,
			     {"initiators" => [],
			      "auth_network" => [ "$network/$cmask" ],
			      "comment" => $comment},
			     undef, \$msg);
    if (!$res) {
	$$msgp = $msg
...
#
# Delete an authorized initiator.
# Returns non-zero if successful, zero otherwise.
#
sub freenasAuthInitDelete($;$)
{
    my ($aindex, $msgp) = @_;

    my $msg = "delete authorized initiator API call failed";
    my $res = freenasRequest("$FREENAS_API_RESOURCE_IST_AUTHI/id/$aindex",
			     "DELETE", undef, undef, undef, \$msg);
    if (!$res) {
	$$msgp = $msg if ($msgp);
	return 0;
...
# Returns the index if successful, 0 otherwise.
#
sub freenasTargetCreate($$$;$$$)
{
    my ($iqn, $aindex, $pgroup, $agroup, $atype, $msgp) = @_;
    $atype = "NONE" if (!defined($atype));

    #
    # There is no longer a separate targetgroup API call.
    # That info is now passed as a list to the target API call.
    #
    my $group = [
	{ "portal" => $pgroup, "initiator" => $aindex,
	  "auth" => $agroup, "authmethod" => $atype
	}
    ];

    my $msg = "create target API call failed";
    my $res = freenasRequest($FREENAS_API_RESOURCE_IST_TARGET,
			     "POST", undef,
			     {"name" => $iqn, "groups" => $group},
			     undef, \$msg);
    if (!$res) {
	$$msgp = $msg if ($msgp);
	return 0;
...
# Returns non-zero if successful, zero otherwise.
#
sub freenasTargetDelete($;$)
{
    my ($tindex, $msgp) = @_;

    #
    # N.B. data string has to be one of "true" or "false" for the force
    # argument, not a hash that with the JSON true or false. It does not
    # default so you cannot leave it off.
    #
    my $msg = "delete target API call failed";
    my $res = freenasRequest("$FREENAS_API_RESOURCE_IST_TARGET/id/$tindex",
			     "DELETE", undef, "false", undef, \$msg);
    if (!$res) {
	$$msgp = $msg if ($msgp);
	return 0;
    }
...
# Returns the index if successful, 0 otherwise.
#
sub freenasTargetExtentCreate($$$;$)
{
    my ($tindex, $eindex, $lunid, $msgp) = @_;

    my $msg = "target to extent associate API call failed";
    my $res = freenasRequest($FREENAS_API_RESOURCE_IST_ASSOC,
			     "POST", undef,
			     {"target" => $tindex,
			      "extent" => $eindex,
			      "lunid" => 0},
...
sub freenasTargetExtentDelete($;$)
{
    ...
    # default so you cannot leave it off.
    #
    my $msg = "delete targetextent API call failed";
    my $res = freenasRequest("$FREENAS_API_RESOURCE_IST_ASSOC/id/$teindex",
			     "DELETE", undef, "false", undef, \$msg);
    if (!$res) {
	$$msgp = $msg if ($msgp);
	return 0;
...
sub listPools() {
    my $poolh = {};
    my $vinfo = freenasRequest($FREENAS_API_RESOURCE_POOL,
			       "GET",  {"limit" => 0});
	...
	if ($vname) {
	    ...
	    warn("*** WARNING: listPools: pool has no name!");
	}
    }
...
sub getZvolsFromVolinfo($)
{
	...
	if (exists($vol->{'type'}) && $vol->{'type'} eq "VOLUME") {
