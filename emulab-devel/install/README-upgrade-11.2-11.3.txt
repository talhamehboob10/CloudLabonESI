Upgrading Emulab servers from FreeBSD 11.2 to 11.3.
...
C. Updating ports/packages
   ...
   Updating the core ports from 11.2 to 11.3 is pretty easy if you were
   running the most recent set of 11.2 packages. Otherwise there is an
...
1. Modify your /etc/pkg/Emulab.conf file, replacing "11.2" with "11.3" in
   the "url" line:
      ...
      sudo sed -i .bak -e 's;/11.2/;/11.3/;' /etc/pkg/Emulab.conf
...
3b. (OPTIONAL) Additional package cleanup.
   ...
   minor release (11.3 instead of 11.2), you can do (on both boss and ops):

     # only do this after you have removed the python2 package above,
...
4. Fix up Apache config for PHP change.
   ...
   If your PHP install got changes from 5.6 to 7.2, you will need to
