...
logopt="--log-long-format --log=/test/log/mysql/base --log-bin=/test/log/mysql/update --log-slow-queries=/test/log/mysql/slowqueries"

if [ $major -eq 5 ]; then
	safeprog=/usr/local/bin/mysqld_safe
	if [ $minor -gt 1 ]; then
	    logopt="--general_log=1 --general_log_file=/test/log/mysql/base"
	    logopt="$logopt --log-bin=/test/log/mysql/update"
	    logopt="$logopt --slow_query_log=1 --binlog-format=MIXED"
	    logopt="$logopt --slow_query_log_file=/test/log/mysql/slowqueries"
