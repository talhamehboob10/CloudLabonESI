...
# them here.
#
# XXX As of V2 API, group info is returned via the TARGET call again.
#
sub targets()
{
	...
	# target group info is in target info
	if (!exists($tgref->{$t})) {
	    my $tgref2 = $tref->{$t}->{"groups"}[0];
	    $tgref->{$t}->{'portalgroup'} = $tgref2->{'portal'};
	    $tgref->{$t}->{'initiatorgroup'} = $tgref2->{'initiator'};
	    $tgref->{$t}->{'authgroup'} = $tgref2->{'auth'};
	    $tgref->{$t}->{'authtype'} = $tgref2->{'authmethod'};
	}
	if (exists($tgref->{$t})) {
	    my $tg = $tgref->{$t};
