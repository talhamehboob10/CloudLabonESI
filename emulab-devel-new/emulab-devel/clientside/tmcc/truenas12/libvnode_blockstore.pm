...
sub exportSlice($$$$) {
    ...
    # Create iSCSI extent.
    #
    $id = freenasExtentCreate($pool, $volume, $iqn, $isro, \$msg);
    if (!$id) {
	warn("*** ERROR: blockstore_exportSlice: $volname: ".
    ...
    #
    my ($eindex, $aindex, $tindex);

    # save the index for making a target association and cleanup
    $eindex = $id;

    #
    # Create an authorized initiator.
    #
    $id = freenasAuthInitCreate($network, $cmask, $tag_ident, \$msg);
    if (!$id) {
	warn("*** ERROR: blockstore_exportSlice: $volname: ".
    ...
    # a target group.
    #
    $id = freenasTargetCreate($iqn, $aindex, $ISCSI_GLOBAL_PORTAL, undef, undef, \$msg);
    if (!$id) {
	warn("*** ERROR: blockstore_exportSlice: $volname: ".
    ...
    $id = freenasTargetExtentCreate($tindex, $eindex, 0, \$msg);
    if (!$id) {
	warn("*** ERROR: blockstore_exportSlice: $volname: ".
...
sub unexportSlice($$$$) {
	...
	if ($associd && !freenasTargetExtentDelete($associd, \$msg)) {
	    warn("*** WARNING: blockstore_unexportSlice: $volname: ".
		 "Failed to disassociate iSCSI target $associd from extent:\n$msg");
	}

	# Remove iSCSI target.
	if ($targetid && !freenasTargetDelete($targetid, \$msg)) {
	    warn("*** WARNING: blockstore_unexportSlice: $volname: ".
		 "Failed to remove iSCSI target $targetid:\n$msg");
	}

	# Remove iSCSI auth group
	if ($authidx && !freenasAuthInitDelete($authidx, \$msg)) {
	    warn("*** WARNING: blockstore_unexportSlice: $volname: ".
		 "Failed to remove iSCSI auth group $authidx:\n$msg");
	}

	# Remove iSCSI extent.
	if ($extentid && !freenasExtentDelete($extentid, \$msg)) {
	    warn("*** WARNING: blockstore_unexportSlice: $volname: ".
